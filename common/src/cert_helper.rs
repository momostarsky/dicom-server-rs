...
pub fn validate_my_certificate(
    ...
    let cert_store = if std::path::Path::new(ca_file).exists() {
        ...
        // 否则使用系统默认的信任库
        openssl::x509::store::X509StoreBuilder::new()?.build()
    };
...
pub fn validate_client_certificate_with_ca(
    ...
    // 检查CA证书文件是否存在
    if !std::path::Path::new(ca_cert_file).exists() {
        return Err(format!("CA证书文件不存在: {}", ca_cert_file).into());
    }
    ...
    // 读取并解析CA证书
    let ca_cert_pem = fs::read(ca_cert_file)?;
    let ca_cert = X509::from_pem(&ca_cert_pem)?;
