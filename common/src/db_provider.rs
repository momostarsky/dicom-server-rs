...
pub trait DbProvider {
    ...
    // 返回值：Some(true) 表示存在，Some(false) 表示不存在，None 表示查询失败
    async  fn patient_study_exists(&self, tenant_id: &str, patient_id:&str, study_uid:&str) -> Option<bool>;
    // 判断DICOM对象是否存在
    // 参数：
    //   - tenant_id: 租户ID
    //   - patient_id: 患者ID
    ...
    // 返回值：Some(true) 表示存在，Some(false) 表示不存在，None 表示查询失败
    async  fn patient_series_exists(&self, tenant_id: &str, patient_id:&str, study_uid:&str, series_uid:&str) -> Option<bool>;
    // 判断DICOM对象是否存在
    // 参数：
    //   - tenant_id: 租户ID
    //   - patient_id: 患者ID
    ...
    // 返回值：Some(true) 表示存在，Some(false) 表示不存在，None 表示查询失败
    async  fn patient_instance_exists(&self, tenant_id: &str, patient_id:&str,study_uid:&str, series_uid:&str, instance_uid:&str) -> Option<bool>;
}
