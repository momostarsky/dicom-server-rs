use actix_web::{post, web, HttpResponse, Result, HttpRequest, http::header};
...
async fn store_instances(
    ...
    log::info!("Received POST request on /studies");
        ...
        Some(ct_header_value) => {
            if let Ok(ct_str) = ct_header_value.to_str() {
                ...
                if !ct_str.to_lowercase().starts_with( MULTIPART_CONTENT_TYPE ) {
                    log::warn!("Invalid Content-Type for STOW-RS: {}", ct_str);
                    return Ok(HttpResponse::UnsupportedMediaType()
                        .body("Content-Type must be multipart/related"));
                }
                log::info!("Content-Type confirmed as multipart/related");
            } else {
                log::error!("Failed to parse Content-Type header value");
    ...
    while let Some(chunk) = payload.next().await {
        match chunk {
            ...
            Err(e) => {
                log::error!("Error reading payload chunk: {}", e);
    ...
    match std::fs::File::create(&filename) {
        Ok(mut file) => {
            if let Err(e) = file.write_all(&file_data) {
                log::error!("Failed to write received data to file {}: {}", filename, e);
                return Ok(HttpResponse::InternalServerError().body("Failed to save data"));
            }
            log::info!("Saved raw multipart/related data to {}", filename);
        }
        Err(e) => {
    ...
    // 这里仅返回 200 OK 作为示意
    log::info!("STOW-RS request processed successfully (simplified)");
    Ok(HttpResponse::Ok()
...
async fn store_instances_to_study(
    ...
    log::info!("Received POST request on /studies/{}", study_instance_uid);
    ...
    if let Some(ct_header_value) = content_type_header {
        if let Ok(ct_str) = ct_header_value.to_str() {
            ...
            log::info!("Content-Type confirmed as multipart/related for specific study");
        } else {
            log::error!("Failed to parse Content-Type header value");
            return Ok(HttpResponse::BadRequest().body("Malformed Content-Type header"));
        }
    ...
    // --- 模拟处理: 将接收到的数据保存到文件 ---
    let filename = format!("received_stow_data_for_study_{}_{}.bin", study_instance_uid, uuid::Uuid::new_v4());
    match std::fs::File::create(&filename) {
        Ok(mut file) => {
            if let Err(e) = file.write_all(&file_data) {
                log::error!("Failed to write received data to file {}: {}", filename, e);
            ...
            log::info!("Saved raw multipart/related data for study {} to {}", study_instance_uid, filename);
        }
        Err(e) => {
            log::error!("Failed to create file {}: {}", filename, e);
            return Ok(HttpResponse::InternalServerError().body("Failed to create storage file"));
    ...
    // 3. 构造简单的成功响应 (同上)
    log::info!("STOW-RS request for study {} processed successfully (simplified)", study_instance_uid);
    Ok(HttpResponse::Ok()
