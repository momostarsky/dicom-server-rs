...
echo -n "838FbMzv^orl0Aol" | openssl pkeyutl -encrypt -pubin -inkey encrypt-public.pem -pkeyopt rsa_padding_mode:pkcs1 -out cipher-key.bin
```

```bash
# echo -n "oLZphu61s%zguTjS" | openssl pkeyutl -encrypt -pubin -inkey x-dicom-public.key -out cipher-iv.bin 
 
echo -n "oLZphu61s%zguTjS" | openssl pkeyutl -encrypt -pubin -inkey encrypt-public.pem -pkeyopt rsa_padding_mode:pkcs1 -out cipher-iv.bin 
```

# 3. 对字符串进行解密
```bash
openssl pkeyutl -decrypt -inkey encrypt-private.key -in cipher-key.bin
openssl pkeyutl -decrypt -inkey encrypt-private.key -in cipher-iv.bin
```
