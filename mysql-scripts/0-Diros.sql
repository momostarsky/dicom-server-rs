...
(
    ...
    PatientSex       VARCHAR(1) COMMENT '患者性别 (0010,0040) M/F/O',
    PatientBirthTime VARCHAR(15) COMMENT '患者出生时间 (0010,0032)，格式 HHMMSS.FFFFFF',
    EthnicGroup      VARCHAR(16) COMMENT '民族 (0010,2160)',

    -- 长文本字段
    AdditionalPatientHistory STRING COMMENT '附加患者历史 (0010,21B0)',
    PatientComments STRING COMMENT '患者注释 (0010,4000)',
    ...
    ENGINE = OLAP
    UNIQUE KEY(tenant_id, PatientID)
    COMMENT "患者实体表，符合DICOM标准，支持多租户"
    DISTRIBUTED BY HASH(tenant_id, PatientID) BUCKETS 10
    PROPERTIES (
                   "replication_num" = "1"
               );
...
(
    ...
    COMMENT "患者实体表，符合DICOM标准，支持多租户"
    DISTRIBUTED BY HASH(tenant_id, StudyInstanceUID) BUCKETS 10
    PROPERTIES (
                   "replication_num" = "1"
               );
...
CREATE TABLE IF NOT EXISTS SeriesEntity
(
    tenant_id                      VARCHAR(64) NOT NULL COMMENT '租户ID',
    -- 主键：序列唯一标识符
    ...
    AcquisitionTime                VARCHAR(15) COMMENT '采集时间 (0008,0032) - TM',
    -- 其他信息
    ...
    COMMENT "患者实体表，符合DICOM标准，支持多租户"
    DISTRIBUTED BY HASH(tenant_id, SeriesInstanceUID) BUCKETS 10
    PROPERTIES (
                   "replication_num" = "1"
               );
...
(
    ...
    ContentTime                            VARCHAR(15) COMMENT '内容时间 (0008,0033) - TM',
    ...
    COMMENT "患者实体表，符合DICOM标准，支持多租户"
    DISTRIBUTED BY HASH(tenant_id, SOPInstanceUID) BUCKETS 10
    PROPERTIES (
                   "replication_num" = "1"
               );